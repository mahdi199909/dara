// Pure data, deliberately with zero imports — src/local/localUser.ts (reachable from the native
// bundle) needs this same list to seed a fresh on-device user, and importing src/lib/defaults.ts
// directly would drag its `import { prisma } from "./db"` along with it into that bundle too.
