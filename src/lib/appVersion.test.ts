...
describe("the announced release", () => {
  ...
  it("lets an admin announce a higher version, force a minimum and use another link", () => {
    const info = resolveAppRelease({ latestVersionCode: 10200, minSupportedVersionCode: 10100, downloadUrl: " https://example.org/app.apk " });
    expect(info).toEqual({ latestVersionName: "1.2.0", latestVersionCode: 10200, minSupportedVersionCode: 10100, downloadUrl: "https://example.org/app.apk" });
  });
