...
//
// The delete and the tombstone are one transaction: a row deleted whose tombstone then failed to be
// written would look, to every phone, as if it had never been deleted — and the next push from a phone
// that still has it would bring it back to life.
...
export async function deleteRowsWithTombstones(userId: string, model: TombstoneModel, where: Record<string, unknown>): Promise<number> {
  return withTransaction(async () => {
    // (resolved inside the transaction: `prisma.<model>` is the transaction's own delegate there)
