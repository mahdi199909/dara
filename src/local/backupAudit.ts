...
/** A duration worth writing down: a finite, non-negative number of milliseconds; anything else is left out. */
...
/** `durationMs`: how long making the file took (the share sheet that follows is the person's time, not the app's). */
export function recordBackupExported(db: LocalDb, userId: string, file: DataExportFile, durationMs?: number): void {
  ...
  log.info("BACKUP_COMPLETED", { rows, tables, layer: "local", durationMs: finiteMs(durationMs) });
}

export function recordBackupImported(db: LocalDb, userId: string, result: ImportResult, durationMs?: number): void {
  ...
  log.log(errors > 0 ? "WARN" : "INFO", errors > 0 ? "RESTORE_PARTIAL" : "RESTORE_COMPLETED", { added, skipped, errors, tables: result.added, layer: "local", durationMs: finiteMs(durationMs) });
}
