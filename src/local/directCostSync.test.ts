...
describe("local directCostSync", () => {
  ...
  it("creates a linked EXPENSE transaction for a task's directCost, auto-creating a default account", () => {
    const db = freshDb();
  ...
  it("updates the existing linked transaction on re-sync instead of duplicating it", () => {
    const db = freshDb();
  ...
  it("soft-deletes the linked transaction when directCost drops to zero", () => {
    const db = freshDb();
    db.run(`INSERT INTO "Task" ("id","userId","title","directCost","incomeAmount","createdAt","updatedAt") VALUES (?,?,?,?,?,?,?)`, ["task_1", USER_ID, "خرید", 10000, 0, now(), now()]);
    syncTaskDirectCostTransaction(db, "task_1");
  ...
  it("keeps income and direct-cost transactions separate for the same task", () => {
    const db = freshDb();
  ...
  it("creates a virtual asset entry for a task logged in a virtual-asset-generating category", () => {
    const db = freshDb();
  ...
  it("creates a linked transaction for an event's directCost", () => {
    const db = freshDb();
