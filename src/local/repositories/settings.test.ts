...
describe("local settings repository", () => {
  ...
  it("creates a default settings row on first get when none exists yet", () => {
    const db = freshDb();
  ...
  it("is idempotent: a second get reuses the same row instead of creating another", () => {
    const db = freshDb();
  ...
  it("persists an update, updates the user's name, and recomputes hourlyValue", () => {
    const db = freshDb();
  ...
  it("an explicit hourlyValueOverride wins over monthlyIncome/workingHoursMonth", () => {
    const db = freshDb();
  ...
  it("stores dashboardCardPrefs as JSON and updates it in place", () => {
    const db = freshDb();
    getSettings(db, USER_ID);
