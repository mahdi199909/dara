...
describe("local assets", () => {
  ...
  it("creates an asset defaulting currentValue to purchasePrice when omitted", async () => {
  ...
  it("updating currentValue creates an AssetTransaction recording the delta", async () => {
    const db = await freshDb();
  ...
  it("does not create an AssetTransaction when currentValue is unchanged", async () => {
    const db = await freshDb();
    const asset = createAsset(db, USER_ID, { name: "لپ‌تاپ", purchasePrice: 50000000, currentValue: 50000000 });
  ...
  it("getAsset includes linked Transaction and AssetTransaction rows", async () => {
    const db = await freshDb();
